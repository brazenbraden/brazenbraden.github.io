...
module Runners
  class Sidekiq
    ...
    def update_display_class(job_class)
      self.class.sidekiq_options.merge!("display_class" => "#{job_class} [#{self.class.name}]")
    end
  end
end
```

In essence, you can now define your background job classes as basic Ruby objects, execute them inline, or transfer them to Sidekiq for background processing. The main requirement is maintaining a consistent DSL with your job classes, such as implementing a mandatory `call` (or equivalent: `perform` or `execute`) method. (Kudos to my friend [Kris](https://github.com/krzyczak) for the original idea.)
